...
func NewMockP2P(peerID string, mockHostManager *MockHostManager, logger logrus.FieldLogger) (*MockP2P, error) {
	...
	return &MockP2P{
		host: &mockHost{
			peerID:   peerID,
			connects: mockHostManager.connects,
		},
...
func GenMockHostManager(peers []string) *MockHostManager {
	...
	for _, v := range filteredPeers {
		connects[v] = make(chan *mockMsg, queueSize)
	}
...
func (m *MockP2P) Broadcast(peerIDs []string, msg []byte) error {
	for _, v := range peerIDs {
		_, exist := m.host.connects[v]
		if !exist {
			return errors.New(fmt.Sprintf("peer id [%s] not exist", v))
		}
	}
	for _, v := range peerIDs {
		msgCopy := make([]byte, len(msg))
		copy(msgCopy, msg)
		connect := m.host.connects[v]
		connect <- &mockMsg{
			stream: &mockStream{
				localPeer:   v,
				remotePeer:  m.PeerID(),
